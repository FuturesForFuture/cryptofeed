...
from cryptofeed.defines import HUOBI_LINEAR_SWAP, FUNDING
...
class HuobiLinearSwap(HuobiDM):
    id = HUOBI_LINEAR_SWAP

    def __init__(self, **kwargs):
        Feed.__init__(self, 'wss://api.hbdm.com/linear-swap-ws', **kwargs)
    ...
    async def _funding(self, pairs):
        async with aiohttp.ClientSession() as session:
            while True:
                for pair in pairs:
                    async with session.get(f'https://api.hbdm.com/linear-swap-api/v1/swap_funding_rate?contract_code={pair}') as response:
